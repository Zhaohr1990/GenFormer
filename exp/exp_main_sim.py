    ...
    def test(self, setting, test=0):
        ...
        if test:
            ...
            if self.args.use_gpu == False:
                self.model.load_state_dict(torch.load(os.path.join(path, 'checkpoint.pth'), map_location=torch.device('cpu')))
            else:
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_state_x, batch_state_y, batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader): 
                batch_state_x = batch_state_x.int().to(self.device)
                batch_state_y = batch_state_y.int().to(self.device)
